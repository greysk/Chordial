...
class ProgramBody(ttk.Frame):
    def __init__(self, midi_dir, title='Chordial'):
        """
        ...
        Chordial program can be run by calling mainloop() on an instance of
        this class.

        Args:
            chords_dir (Pathlike): Path to directory containing chord
                progression .mid file.
            title (str, optional): Title for root tk window.
    ...
    def _create_widgets(self):
        ...
        tk.Label(self.master, text="file name", textvariable=self.file_label,
    ...
    def remove(self):
        ...
        """
        if self.file_to_open is not None:
            self.sound_files.remove(self.file_to_open)


if __name__ == '__main__':
    chordial_app = ProgramBody(midi_dir=CHORDIAL)
